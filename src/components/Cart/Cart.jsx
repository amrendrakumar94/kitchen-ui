...
function Cart() {
    ...
    if (userId > 0) {
      const fetchData = async () => {
        const response = await fetch(
          `http://alb-springboot-1620036472.eu-north-1.elb.amazonaws.com/get-dish-details-by-user-id/${userId}`
        );
  ...
  const handleIncrement = async (dishId) => {
    try {
      const response = await fetch(
        `http://alb-springboot-1620036472.eu-north-1.elb.amazonaws.com/increment-cart-item/${user.id}/${dishId}`,
        {
          method: "POST",
        }
      );
  ...
  const handleDecrement = async (dishId) => {
    try {
      const response = await fetch(
        `http://alb-springboot-1620036472.eu-north-1.elb.amazonaws.com/decrement-cart-item/${user.id}/${dishId}`,
        {
          method: "POST",
        }
      );
  ...
  const removeItem = async (dishId) => {
    try {
      const response = await fetch(
        `http://alb-springboot-1620036472.eu-north-1.elb.amazonaws.com/remove-cart-item/${user.id}/${dishId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleOrder = async () => {
    try {
      ...
      const response = await fetch("http://alb-springboot-1620036472.eu-north-1.elb.amazonaws.com/save-order-details", {
        method: "POST",
        headers: {
